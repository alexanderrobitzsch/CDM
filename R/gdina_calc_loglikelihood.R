...
gdina_calc_loglikelihood <- function(delta_vec, beta, attr.prob, Z, delta_indices, J,
        ...
        resp.ind.list, zeroprob.skillclasses, item.patt.freq, loglike, G,
        reduced.skillspace )
{
    ...
    # reconvert delta
    delta <- delta.new <- gdina_delta_convert_into_list( delta_vec=delta_vec,
                                delta_indices=delta_indices, J=J)
    # beta parameter
    if ( ! is.null(beta) ){
        if (G==1){
    ...
    p.xi.aj <- gdina_calc_individual_likelihood( IP=IP, L=L, pjM=pjM,
                        item.patt.split=item.patt.split, J=J,
                        resp.ind.list=resp.ind.list,
                        zeroprob.skillclasses=zeroprob.skillclasses )
    ll <- gdina_calc_deviance( p.xi.aj=p.xi.aj, attr.prob=attr.prob,
