		...
		random_starts)
{
	...
	if ( is.null(class_probs_init) ){
		class_probs <- reglca_init_parameters_class_probs( nclasses=nclasses, sd_noise_init=sd_noise_init)
	} else {
	...
	if ( is.null(item_probs_init) ){	
		item_probs <- reglca_init_parameters_item_probs( qmeans=qmeans, I=I, nclasses=nclasses, sd_noise_init=sd_noise_init, parm_range=1 ) 	
	} else {
	...
	if (random_starts > 1){
		class_probs <- list()
		for (rr in 1:random_starts){
			class_probs[[rr]] <- reglca_init_parameters_class_probs( nclasses=nclasses, sd_noise_init=sd_noise_init)
		}
